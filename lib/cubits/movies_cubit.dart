...
//   });
//   @override
//   List<Object?> get props => [movies, hasMore, isOffline];
// }
//
...
//   Future<void> toggleSave(MovieModel movieId) async {
//     await _repo.toggleSave(userId: activeUserId, movieId: movieId, movie: null);
//
...
//     );
//
//     print("movies: #${_movies.length} @Active UserId: $activeUserId");
...
class MoviesCubit extends Cubit<MoviesState> {
  ...
  Future<void> fetchMore() async {
    ...
    if (current is MoviesLoaded && !current.hasMore) {
      return;
    }
  ...
  Future<void> toggleSave(
    ...
    try {
      ...
      if (idx == -1) {
        print("Movie not found in local list");
        return;
      }

      final currentMovie = _movies[idx];

      final isSaved = !currentMovie.isSavedByActiveUser;

      _movies[idx] = currentMovie.copyWith(
        isSavedByActiveUser: isSaved,
        saveCount: isSaved ? currentMovie.saveCount + 1 : currentMovie.saveCount - 1,
      );
      ...
      print("Movies Cubit Toggle Error: $e");
